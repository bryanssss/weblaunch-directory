...
export async function onRequest(context) {
  ...
  if (context.env.DB) {
    try {
      const result = await context.env.DB.prepare(`
        SELECT id, name, slug, normalized_domain, description, category, approved_at
  ...
  const items = sites.map((site) => `
    <item>
      ...
      <link>${xmlEscape(`${origin}/site.html?domain=${encodeURIComponent(site.normalized_domain)}`)}</link>
      <guid>${xmlEscape(`${origin}/site.html?domain=${encodeURIComponent(site.normalized_domain)}`)}</guid>
